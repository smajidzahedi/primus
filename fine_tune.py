...
def objective(trial):
    ...
    if config["policy_type"] == "ac_policy":  
        config['lr_actor'] = trial.suggest_categorical("lr_actor", [1e-4, 2e-4, 3e-4, 4e-4, 5e-4, 6e-4, 7e-4, 8e-4, 9e-4])
        config['lr_critic'] = config['lr_actor'] * beta
        config['l1_out_l2_in_actor'] = trial.suggest_categorical("l1_out_l2_in_actor", [64, 128, 256])
        config['l2_out_l3_in_actor'] = trial.suggest_categorical("l2_out_l3_in_actor", [64, 128, 256])
        config['l1_out_l2_in_critic'] = trial.suggest_categorical("l1_out_l2_in_critic", [64, 128, 256])
        config['l2_out_l3_in_critic'] = trial.suggest_categorical("l2_out_l3_in_critic", [64, 128, 256])
        decay_factor = 1 - alpha * config['lr_actor']
    elif config["policy_type"] == "q_learning":
        config["l1_out_l2_in_q"] = trial.suggest_categorical("l1_out_l2_in_q", [64, 128, 256])
        config["l2_out_l3_in_q"] = trial.suggest_categorical("l2_out_l3_in_q", [64, 128, 256])
        config["lr_q"] = trial.suggest_categorical("lr_q", [1e-4, 2e-4, 3e-4, 4e-4, 5e-4, 6e-4, 7e-4, 8e-4, 9e-4])
        decay_factor = 1 - alpha * config["lr_q"]
    # Update the parameters in the config
    config['servers_config']['recovery_cost'] = trial.suggest_int("recovery_cost", 5, 30, 1)
...
if config["policy_type"] == "ac_policy":  
    ...
    config['l2_out_l3_in_actor'] = best_params["l2_out_l3_in_actor"]
    config['l1_out_l2_in_critic'] = best_params["l1_out_l2_in_critic"]
    config['l2_out_l3_in_critic'] = best_params["l2_out_l3_in_critic"]
    ...
    config["l2_out_l3_in_q"] = best_params["l2_out_l3_in_q"]
