...
def calculate_app_state_values(server_state_len, trans, dff):
...
def compute_returns(dff, rs, next_state_v):
    r = next_state_v
    rts = []
...
if __name__ == "__main__":
    ...
    lr = 0.1
    epsilon = 0.1
    policy = policies.QLPolicy(dim, df, lr, epsilon)
    server = servers.QLServer(0, policy, app, servers_config, utility_normalization_factor)
    ...
    for i in range(1, 100000):
        ...
        if i % 500 == 0:
            actions = []
            for j in range(0, app.get_state_space_len()):
                state = (0, j)
                print(server.policy.printable_action(state), end="")
            print()
